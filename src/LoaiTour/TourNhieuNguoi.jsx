...
//         const filteredTours = response.data.filter(tour => tour.maLoai === "LT03");
...
const TourNhieuNguoi = () => {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        const response = await axios.get('https://tourdulich-bheqa4hpbgbjdrey.southeastasia-01.azurewebsites.net/tour/Tour');
        // Lọc chỉ lấy những tour có maLoai là "LT03" và trangThai là "Còn hạn"
        const filteredTours = response.data.filter(tour => tour.maLoai === "LT03" && tour.trangThai === "Còn hạn");
        setToursData(filteredTours); 
      } catch (err) {
        setError(err); 
      }
